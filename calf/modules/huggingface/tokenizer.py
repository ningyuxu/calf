...
class HuggingfaceTokenizer:
    ...
    def __init__(self,
                 ...
                 **kwargs) -> None:
        try:
            self.backend = AutoTokenizer.from_pretrained(model_name,
    ...
    def __call__(self, text: Union[str, List[str]]) -> Dict:
        ...
        tokenized_text = list(itertools.chain.from_iterable([w for w in tokenized_text]))

        # input_ids
        input_ids = torch.tensor(self.backend.convert_tokens_to_ids(tokenized_text)).long()
    ...
    def add_special_tokens(self, special_tokens_dict: Dict[str, Union[str, AddedToken]]) -> int:
        """
        Add a dictionary of special tokens (eos, pad, cls, etc.) to the encoder. If special tokens
        are NOT in the vocabulary, they are added to it (indexed starting from the last index of the
        current vocabulary).
        Args:
            special_tokens_dict (`Dict`):
                The dictionary containing special tokens. Keys should be in the list of predefined
                special attributes:
